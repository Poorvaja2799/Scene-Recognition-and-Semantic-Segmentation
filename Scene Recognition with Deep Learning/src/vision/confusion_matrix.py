...
def generate_confusion_data(
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, **dataloader_args) # TODO: might have to set shuffle to False
    ...
    # build class label list such that index -> class name
    ...
    # Run inference in no_grad mode and fill arrays in loader order
    with torch.no_grad():
        # use the model's actual device (works whether it's on CPU, CUDA, or MPS)
        device = next(model.parameters()).device
        offset = 0
        for (x, y) in loader:
            # match device + dtype
            x = x.to(device=device, dtype=torch.float32, non_blocking=cuda)
            y = y.to(device=device, dtype=torch.long, non_blocking=cuda)

            logits = model(x)
            p = torch.argmax(logits, dim=1)

            bs = x.shape[0]
            start, end = offset, offset + bs
            preds[start:end] = p.cpu().numpy().astype(np.int32)
            targets[start:end] = y.cpu().numpy().astype(np.int32)
            offset = end

  
    model.train()

    # preds and targets are already numpy arrays; return them directly
...
def generate_confusion_matrix(
    ...
    for target, prediction in zip(targets, preds):
        ...
        # increment the cell corresponding to (ground truth row, predicted col)
        t = int(target)
        p = int(prediction)
    ...
    if normalize:
        ...
        # normalize each row by the number of true instances of that class
        # sum per ground-truth row (shape: (num_classes, 1))
        row_sums = confusion_matrix.sum(axis=1, keepdims=True)
        # boolean mask for rows that actually occur
        nz_mask = (row_sums[:, 0] != 0)
        if np.any(nz_mask):
            # divide each non-empty row by its row sum; row_sums[nz_mask] has shape (m,1)
            confusion_matrix[nz_mask, :] = confusion_matrix[nz_mask, :] / row_sums[nz_mask]
...
def generate_accuracy_data(
    ...
    batch_size = 32
    cuda = use_cuda and torch.cuda.is_available()
    device = torch.device("mps") if (use_cuda and torch.backends.mps.is_available()) else torch.device("cpu")
    model.to(device)
    dataloader_args = {"num_workers": 1, "pin_memory": True} if cuda else {}
    ...
    # Run inference in no_grad mode and fill arrays in dataset order
    with torch.no_grad():
        offset = 0
        for (x, y) in loader:
            if cuda:
            ...
            logits = model(x)
            # # apply sigmoid for multilabel probabilities and threshold at 0.5
            # probs = torch.sigmoid(logits)
            # p = (probs >= 0.5).cpu().numpy().astype(np.int32)
            # t = y.cpu().numpy().astype(np.int32)

            p = (logits >= 0.5).to(torch.int32).cpu().numpy()
            t = (y >= 0.5).to(torch.int32).cpu().numpy()

            start = offset
            end = offset + batch_size_eff
            preds[start:end, :] = p
            targets[start:end, :] = t
            offset = end
...
def generate_accuracy_table(
    ...
    # targets and preds are (N, k) arrays of 0/1 values
    ...
    # safety: if empty, return zeros
    if t.size == 0:
        return accuracy_table

    # compute per-attribute accuracy: fraction of samples where prediction equals target
    # shape: (k,)
    correct = (t == p).astype(np.float32)
    # mean across samples (axis=0)
    accuracy_table = correct.mean(axis=0)
